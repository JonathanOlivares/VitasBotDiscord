...
class Options(commands.Cog):
    ...
    async def options(self, ctx):
        ...
        for command, info in util.commands_option_ES.items():
    ...
    async def language(self, ctx, lang: str):
        if lang in self.valid_language:
            ...
            await var.unload("ES", self.bot)
            await asyncio.sleep(2)
            await var.load(self.bot)
            await var.bot_send_msg(ctx, "Idioma cambiado.")
    ...
    async def channel_bot_txt(self, ctx, channel_name: str):
        ...
        if channel == None:
            msg = "El canal de texto ingresado no existe"
